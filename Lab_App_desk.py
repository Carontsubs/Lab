...
def main(page: ft.Page):
    ...
    res_zirconi = ft.Text(size=18, weight="bold", color="blue")
    def calc_zirconi(e):
        try:
    ...
    def calc_flux(e):
        try:
            ...
            s, t = 2.55*(r/c), c+r
            res_flux.controls = [
                ft.Text(f"Zn: {round(b,1)} | ZnCl2: {round(c,1)} | NH4Cl: {round(r,1)}", size=18, weight="bold", color="blue"),
                ft.Text(f"SALS TOTALS: {round(t,1)} | RELACIÓ: {round(s,2)}", size=18, weight="bold", color="blue")
            ]; page.update()
        except: res_flux.controls = [ft.Text("Error", color="red")]; page.update()
    ...
    res_tasa = ft.Text(size=18, weight="bold", color="blue")
    def calc_tasa(e):
        try:
    ...
    res_pes = ft.Text(size=18, weight="bold", color="blue")
    def calc_pes(e):
        try:
    ...
    def calc_mora(e):
        try:
            ...
            t = 600*(D-(1+((r+b)/1350)))
            res_mora.controls = [
                ft.Text(f"Cu: {round(c,1)} | PiroCu: {round(b,1)} | 4K: {round(r,1)}", size=18, weight="bold", color="blue"),
                ft.Text(f"Orto: {round(t,1)}", size=18, weight="bold", color="blue")
            ]; page.update()
        except: res_mora.controls = [ft.Text("Error", color="red")]; page.update()
    ...
    res_blue = ft.Text(size=18, weight="bold", color="blue")
    def calc_blue(e):
        try:
    ...
    res_s675 = ft.Text(size=18, weight="bold", color="blue")
    def calc_s675(e):
        try:
    ...
    def calc_mv_logic(e):
        try:
            ...
            res_mv.controls = [
                ft.Text(f"Consum: {round(Cl, 2)} ml/l", size=18, weight="bold", color="blue"), 
                ft.Text(f"CLORURS: {round(Cl * 14.2, 1)} ppm", size=18, weight="bold", color="blue")
            ]; page.update()
        except: res_mv.controls = [ft.Text("Error", color="red")]; page.update()
    ...
    def calc_dil_logic(e):
        try:
            ...
            res = [ft.Text(f"Volum Final: {round(v_fin, 2)} ml", size=18, weight="bold", color="blue")]
            for n, p in dil_list: res.append(ft.Text(f"Afegir {n}: {round((p/100)*v_fin, 2)} ml", size=18, weight="bold", color="blue"))
            res_dil.controls = res; page.update()
        except: res_dil.controls = [ft.Text("Error", color="red")]; page.update()
